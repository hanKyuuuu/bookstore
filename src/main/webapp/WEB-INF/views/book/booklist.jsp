	...
	<table class="table table-sm table-bordered">
		...
		<c:forEach items="${list}" var="book" varStatus="sts">
			<tr>
				<td>${sts.count}</td>
				<td><a href="book?
				cmd=view&bno=${book.bno}&page=${pVo.page}&searchword=${pVo.searchword}&searchtype=${pVo.searchtype}">${book.title}</a></td>
				<td>${book.writer}</td>
	     ...
	     <nav aria-label="Standard pagination example">
	          <ul class="pagination">
	           ...
	           <c:forEach begin="${pVo.beginPage}" end="${pVo.endPage}" var="i">
			 		<c:choose>
			 			<c:when test="${i!=pVo.page}"><li class="page-item"><a class="page-link" href="book?
		              cmd=list&page=${i}&searchword=${pVo.searchword}&searchtype=${pVo.searchtype}">${i}</a></li></c:when>
		 		...
		 		<c:if test="${pVo.next}">
		           <li class="page-item">
		              <a class="page-link" href="book?
		              cmd=list&page=${pVo.endPage+1}&searchword=${pVo.searchword}&searchtype=${pVo.searchtype}" aria-label="Next">
